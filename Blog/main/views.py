...
def add_post_view(request: HttpRequest):
    ...
    if request.method == 'POST':
        try:
            new_post = Post(title=request.POST["title"], content=request.POST["content"], is_published=request.POST.get("is_published", False), poster=request.FILES["poster"] , category=request.POST["category"])
            new_post.save()
